...
public class RentalUIController {
  ...
  public ResponseEntity<List<AvailableQuantity>> getVenueSelect(
      @PathVariable String name, @PathVariable @DateTimeFormat(pattern = "yyyy-MM-dd") Date date) {
  ...
  public void insertVenueList(
      @RequestBody @Valid VenueListRequest venueListRequest, Principal principal)
  ...
  public boolean checkIVL(@RequestBody @Valid Map<String, String> bean, Principal principal)
      throws ParseException {
    User user = userService.findByUsername(principal.getName());
    ...
    if (bean != null) {
      ...
      if (rental != null) {
        VenueList venueList =
            rentalService.findByRentalAndVenueAndRentalDateAndPeriod(
  ...
  public ResponseEntity<Rental> orderRental(@PathVariable Integer rentalId) {
    ...
    if ("場地".equals(r.getType())) {
      ...
      //            try {
      //                emailService.sendRentalMail(email, "[Bake Your Life 烘焙材料網]
      // 租借單下單成功通知",r,"rentaldown_t");
      //            } catch (MessagingException e) {
      //                throw new RuntimeException(e);
      //            }
    }
