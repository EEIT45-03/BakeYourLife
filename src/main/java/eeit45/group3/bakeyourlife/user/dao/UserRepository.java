...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT CONVERT(char(7), u.register_time, 111) AS 'label', count(u.register_time) AS 'value' FROM users u  GROUP BY CONVERT(char(7), u.register_time, 111) ORDER BY CONVERT(char(7), u.register_time, 111)")
