...
public interface FarmerProductRepository extends JpaRepository<FarmerProductBean, Integer> {
  ...
  List<FarmerProductBean> findByStateAndFarmerFarmerIdOrderByLaunchedTimeDesc(
      Integer state, Integer farmerId);
  ...
  List<FarmerProductBean> findByTypeAndStateAndFarmerProductIdNotOrderByLaunchedTimeDesc(
      String type, Integer state, Integer farmerProductId);

  // 多個商品 從類型跟狀態跟廠商ID找 按時間排序 新到舊
  List<FarmerProductBean> findByTypeAndStateAndFarmerFarmerIdOrderByLaunchedTimeDesc(
      String type, Integer state, Integer farmerId);
  ...
  @Query(
      nativeQuery = true,
      value =
          "select TOP 1 oi.product_name+'@'+convert(varchar,format((sum(oi.qty)),'N0')) from order_item oi "
  ...
  @Query(
      nativeQuery = true,
      value =
          " SELECT CONVERT(char(7), o2.label, 111) AS 'label',  isnull(sum(o2.sub_total)/10000,0)   AS 'value' FROM "
  ...
  @Query(
      nativeQuery = true,
      value =
          " select fn.farmer_name AS 'label',isnull(sum(oi.sub_total)/10000,0) AS 'value'  from order_item oi "
  ...
  @Query(
      nativeQuery = true,
      value = "Select type AS 'label', count(type) AS 'value' from farmer_product group by type")
  ...
  @Query(
      nativeQuery = true,
      value =
          "Select type AS 'label', count(type) AS 'value' from farmer_product WHERE farmer_id =? group by type")
  ...
  @Query(
      nativeQuery = true,
      value =
          "select isnull(sum(oi.sub_total)/10000,0) from order_item oi join ( select 'F'+convert(varchar,farmer_product_id) AS 'id' from farmer_product  where farmer_id =?) f on oi.product_no =f.id join(select order_id,order_status from orders where order_status != '已退款' AND order_status != '已取消') o on oi.order_id = o.order_id")
  ...
  @Query(
      nativeQuery = true,
      value =
          "select format(isnull(sum(oi.sub_total)/10000,0),'N0') from order_item oi join ( select 'F'+convert(varchar,farmer_product_id) AS 'id' from farmer_product) f on oi.product_no =f.id join(select order_id,order_status from orders where order_status != '已退款' AND order_status != '已取消') o on oi.order_id = o.order_id")
  String saleAmount();
  ...
  @Query(
      nativeQuery = true,
      value =
          " select isnull(CAST(AVG(CONVERT(FLOAT,star)) AS DECIMAL(10,1)),0) from product_comment where farmer_product_id  is not null")
  Float avgStar();
  ...
  @Query(
      nativeQuery = true,
      value =
          "  select isnull(CAST(AVG(CONVERT(FLOAT,c.star)) AS DECIMAL(10,1)),0) from product_comment c join( select farmer_product_id from farmer_product where farmer_id =?) f on c.farmer_product_id =f .farmer_product_id")
  ...
  @Query(
      nativeQuery = true,
      value =
          "  select Top 6  f1.* from farmer_product f1 "
