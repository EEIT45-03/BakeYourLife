...
public class AdminTackleController {
  ...
  public String viewIndex(
      @RequestParam(value = "tName", required = false) String tackleName,
    ...
    if ((tackleName != null && tackleName.length() > 0) && (sort != null && sort.length() > 0)) {
      // 設置給JSP使用
      model.addAttribute(
          "tackles", tackleService.findAllByTackleNameAndTackleSort(tackleName, sort));
  ...
  //                               @RequestParam(value = "tackleImage", required = false)
  // MultipartFile[] file,
  ...
  public String updateTackle(
      @RequestParam Integer tackleId,
