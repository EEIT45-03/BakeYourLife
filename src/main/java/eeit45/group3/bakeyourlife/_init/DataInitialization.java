...
public class DataInitialization implements ApplicationListener<ContextRefreshedEvent> {
  ...
  public void onApplicationEvent(ContextRefreshedEvent event) {
    ...
    try {
      fakeUserDatas =
          objectMapper.readValue(resourceAsStream, new TypeReference<List<FakeUserData>>() {});
    } catch (IOException e) {
    ...
    for (int i = 0; i < 365; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        ...
        user.setRegisterTime(
            new Timestamp(date.getTime() + (long) (Math.random() * (82800000 - 28800000))));
        users.add(user);
      }
                      ...
                      e -> {
                        ...
                        if (e.getProductNo().charAt(0) == 'F') {
                          FarmerProductBean farmerProduct =
                              (FarmerProductBean) cartItemsMap.get(e.getProductNo());
                          // 銷售紀錄
                          SalesRecord salesRecord = new SalesRecord();
