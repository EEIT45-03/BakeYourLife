...
public class ShoppingCartController {
  ...
  public ResponseEntity<Cart> viewCart(
      @ModelAttribute Cart cart, Authentication authentication, ModelMap model) {
  ...
  public ResponseEntity<Cart> cartAdd(
  ...
  public ResponseEntity<Cart> cartRemove(
      @RequestParam String itemNo, @ModelAttribute Cart cart, Authentication authentication) {
    User currentUser = null;
    if (authentication != null) {
      currentUser = userService.getCurrentUser(authentication);
    }
  ...
  public ResponseEntity<Cart> cartUpdate(
  ...
  public ResponseEntity<String> checkOut(
          ...
          .forEach(
              (e) -> {
                if (e.getProductNo().charAt(0) == 'G') {
                  Goods goods =
                      goodService.getGoods(Integer.parseInt(e.getProductNo().substring(1)));
    ...
    return ResponseEntity.status(HttpStatus.FOUND)
        .location(URI.create(baseURL + "/Order/" + orderNo + "/Pay"))
        .build();
  }

  @GetMapping("/Cart/useCoupon")
  public ResponseEntity<Cart> useCoupon(
      @ModelAttribute Cart cart, @RequestParam String code, Authentication authentication) {
    Coupon coupon = couponService.findById(code).orElse(null);
    Date now = new Date();
    if (coupon == null
        || !"進行中".equals(coupon.getState())
        || !(coupon.getMaxQuantity() > coupon.getUsedQuantity())) {
  ...
  public ResponseEntity<Cart> removeCoupon(
      @ModelAttribute Cart cart, Authentication authentication) {
    User currentUser = userService.getCurrentUser(authentication);
  ...
  private ResponseEntity<Cart> getCartResponseEntity(
      @RequestParam Integer qty, Cart cart, CartItem cartItem, User currentUser) {
    Cart cartRedis = null;
    if (currentUser != null) {
