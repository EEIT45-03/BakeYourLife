...
public interface OrderRepository extends DataTablesRepository<Order, Integer> {
  ...
  @Query(
      nativeQuery = true,
      value =
          "SELECT TOP(15) ot.product_no AS 'id',ot.product_name AS 'label', SUM(ot.sub_total)/10000 'value' FROM order_item ot LEFT JOIN orders o ON ot.order_id = o.order_id WHERE o.order_status != '已退款' AND O.order_status != '已取消' GROUP BY ot.product_no,ot.product_name ORDER BY  SUM(ot.sub_total)/10000 desc")
  List<ProductSaleAmount> findProductSaleAmount();

  @Query(
      nativeQuery = true,
      value =
          "SELECT CONVERT(char(7), o.order_date, 111) AS 'label', sum(o.total_price)/10000 AS 'value' FROM orders o WHERE o.order_status != '已退款' AND o.order_status != '已取消' GROUP BY CONVERT(char(7), o.order_date, 111) ORDER BY CONVERT(char(7), o.order_date, 111)")
  List<ProductSaleAmount> findMonthSaleAmount();

  @Query(
      nativeQuery = true,
      value =
          "SELECT sum(o.total_price)/10000 AS 'value' FROM orders o WHERE o.order_status != '已退款' AND o.order_status != '已取消' AND year(o.order_date) = YEAR(GETDATE())")
